...
namespace XXF.Db.DbSubscribeRuleConfig
{
    public class DbSubscribeRuleConfigHelper
    {
 //缓存key标识
        private static string configKey = "DbSubscribeRuleConfig"; //+ DateTime.Now.ToString("yyyyMMddHHmmss");
        private static readonly int intervalTime = 10;//单位秒
        public static Dictionary<string, tb_dbsubscribe_config_model> configData = new Dictionary<string, tb_dbsubscribe_config_model>();
        public static XXF.Cache.ConfigCacheProvider<List<tb_dbsubscribe_config_model>> configCacheProvider = new XXF.Cache.ConfigCacheProvider<List<tb_dbsubscribe_config_model>>(configKey, RefreshData, RefreshDataByLocalFile, intervalTime);
        ...
        /// <returns></returns>
        private static void RefreshDataByLocalFile(List<tb_dbsubscribe_config_model> dic)
        {
        ...
        {
            string keyTemp = "DbSubscribeRuleConfig_{0}";
        ...
        //        conn.Open();
        //     //   int type=LibConvert.StrToInt(XXF.Common.XXFConfig.MainConnectString):
        //        var list = new Dal.tb_dbsubscribe_config_dal().Get(conn);
        //        return list;
        //    }
        //}
        ...
        /// </summary>
        /// <returns></returns>
        public static List<Model.tb_dbsubscribe_config_model> GetCacheConfig()
        {
            if (configData.Count() == 0)
            {
                return new List<tb_dbsubscribe_config_model>();
            }
            List<tb_dbsubscribe_config_model> models = new List<tb_dbsubscribe_config_model>(configData.Values);
            return models;
            //var o = Cache.CacheFileProvider.GetCache(cachekey);
            //if(o == null)
            //    return new List<Model.tb_dbsubscribe_config_model>();
            //else
            //    return (o as Cache.CacheFileInfo).CacheData as List<Model.tb_dbsubscribe_config_model>;
        }
    }
}
